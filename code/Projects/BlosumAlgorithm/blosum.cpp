...
void blosum::fill_block(FILE* fdat)
{
...
void blosum::read_parameters(FILE* fdat, int argc,char *argv[]){
    char ctemp[6];
    int itemp;
...
void blosum::calculate_matrix() {
    FILE *fdat, *fout;
    char outfile[FNAMELEN];
...
int blosum::aachar_to_num(char ch)
{
  switch (ch) {
    case 'A': return(0);
    case 'R': return(1);
...
char *blosum::num_to_aachar(int num)
{
  switch (num) {
    case 0: return("A");
    case 1: return("R");
