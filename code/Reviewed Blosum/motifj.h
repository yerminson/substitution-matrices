...
/*  6/29/90 J. Henikoff
 >>>>>>>>>>>>>>>>>>>>>>>>>Blocks 6.0 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    2/18/93  Added pir field to db_id structure.
    7/19/93  Added MAXTITLE
 >>>>>>>>>>>>>>>>>>>>>>>>>Blocks 6.2 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  10/24/93  Changed type of seq_no[] array in struct motif from
            unsigned char to int. Also freq and dups.
            Changed VERSION from 6 to 7.
  11/ 4/93  Changed MAX_MERGE_WIDTH from 60 to 55 to prevent wrapping.
  11/ 7/93  Changed HIGHPASS from 8 to 4 for blosum62 1/2 bit.
 >>>>>>>>>>>>>>>>>>>>>>>>>Blocks 7.x<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   7/27/94  Changed MAXSEQS from 400 to 550 for blocks 8.0
 >>>>>>>>>>>>>>>>>>>>>>>>>Blocks 8.0<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   7/31/94  Changed MAXSEQS from 550 to 700 for PS01033
...
#define MAXSEQS       700   /* Max number of sequences to be analyzed */
#define MINSEQS           2     /* Min number of sequences to be analyzed */
#define MAXFREQ           700   /* Max occurences of motif in all seqs */
